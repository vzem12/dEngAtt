...
class UserInfo():
  ...
  def userGen(self):
    ...
    if not self.state:
      self.watch_id += 1
      try:
        self.channel_id = random.choice(random.choices(self.channels_dif[0],self.channels_dif[1])[0])
      except:
         print(self.channels_dif)
      delta = 7200
    else:
      ...
      #Выключение на 7 days (от 0 до 7), 48 часов, переключение каналов
      delta = random.randint(1,random.choices([604800, 172800, 5],weights)[0])
